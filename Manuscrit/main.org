...
*** TODO Lorsque je cite mes papiers, les citer tous - modifier le bib.
*** TODO Virer les <-> dans les données pour la régression
*** TODO réécrire la partie qui commence par il est facile de voir dans la démo de SCIRL
...
\begin{align}
\quality{\expertpolicy}{\rewardfunc_{\paramirl}}(s,a)&=\expectationknowing{\sum\limits_{\timeindex = 0}^{\infty} \discount^{\timeindex}\rewardfunc_{\paramirl}(\state_{\timeindex},\action_{timindex})}{\state_0 = \state,a_0=a,\policy}\\
...
Maintenant que le principe de \gls{scirl} ainsi qu'une instanciation particulière utilsant l'\autoref{alg:taskar} et \gls{lstdmu} sont en place, intéressons-nous aux garanties théoriques qu'il est possible de fournir sur les performances d'imitation de \gls{scirl}.
...
\text{ avec } c(t) &=
  \max_{\pi_1,\dots,\pi_t,s\in\statespace}\frac{(\rho_E^\top \transprobmat{\pi_1}\dots
...
\begin{theorem}[Borne des performances de \glstext{scirl}]
  ...
  \begin{equation}
    0\leq
    \expectationknowing{V^{\pi^*_{R_{\theta}}}_{R_{\theta}}-V^{\expertpolicy}_{R_{\theta}}}{s\sim\rho_E}
    \leq \frac{C_f}{1-\gamma}\left(\bar{\epsilon}_Q +
    \classiferror{\expertdistrib}\frac{2\gamma\|R_{\theta}\|_\infty}{1-\gamma}
    \right).
  \end{equation}
\end{theorem}
La preuve, due au travail de Matthieu Geist[fn::Co-auteur des contributions concernant \gls{scirl} \citep{klein2012sctructured}.] est reportée en \autoref{hier-demo}.
...
# <<hier:expmountaincar>>
    ...
    Le classifieur dont les performances sont illustrées est une \gls{svm} à noyau gaussien dont les paramètres sont choisis automatiquement par l'implémentation utilisée[fn::\url{http://scikit-learn.org/stable/modules/generated/sklearn.svm.SVC.html}]. Les performances de cette technique étant très légèrement supérieures à ceux du classifieur structuré de \citet{taskar2005learning} (\autoref{alg:taskar}), ce sont celles que nous affichons ici.
...
%If $\pi=\expertpolicy$, we have, thanks to the power series expression of $(I-\gamma \transprobmat{\expertpolicy})^{-1}$, the definition of $\rho_E$ and the definition of the %$\mu$-weighted $L_1$ norm, one property of which is that $\forall X, \mu^TX \leq \|X\|_{1,\mu}$:
...
*** mountain car
Le protocole expérimental est le même que dans la \autoref{hier-expmountaincar}. Le classifieur choisi pour \gls{cascading} est le même que celui dont les performances sont illustrées \autoref{fig:csimountaincar}. Il s'agit de l'implémentation d'une SVM[fn::\url{http://scikit-learn.org/stable/modules/generated/sklearn.svm.SVC.html}]. Le noyau est gaussien, les paramètres par défaut sont utilisés. Nous sommes dans un cas où l'ingénierie déployée par l'opérateur est minimale. Le régresseur est du même tonneau, il s'agit d'une SVR[fn::\url{http://scikit-learn.org/stable/modules/generated/sklearn.svm.SVR.html}] utilisée avec ses paramètres par défaut. 
...
Nous allons décomposer le terme $V^{\pi^*_{R_{\theta}}}_{R_{\theta}}-V^{\expertpolicy}_{R_{\theta}}$ (qui est positif ou, au mieux, nul, puisque $\pi^*_{R_{\theta}}$ est optimale) afin de faire apparaître le résidu de Bellman $\expectationknowing{\bellmanoptopeval{\rewardfunc_{\theta}}{V^{\expertpolicy}_{R_{\theta}}}(s)-V^{\expertpolicy}_{R_{\theta}}(s)}{s\sim\rho_E}$. On rappelle que les définitions des opérateurs d'optimalité et d'évaluation de Bellman $\bellmanoptopeval{\rewardfunc}{}$ et $\bellmanevalopeval{\pi}{\rewardfunc}{}$ sont données équations \ref{eq:bellmanoptop} et \ref{eq:bellmanevop}.
  \begin{equation}
...
-> Dont nous donnons l'énoncé theoreme truc.
\begin{equation}
...
D'après la définition de $c(t)$ (\autoref{eq:ctdef}) et du coefficient de concentration $\ccoeff$ (\autoref{eq:ccoeffdef}) :
  \begin{align}
    0\leq\expectationknowing{V^{\pi^*_{R_{\theta}}}_{R_{\theta}}(s)-V^{\expertpolicy}_{R_{\theta}}(s)}{s\sim\rho_E} &\leq \sum_{t\geq 0}c(t)
    \expectationknowing{\bellmanoptopeval{\rewardfunc_{\theta}}{}V^{\expertpolicy}_{R_{\theta}}(s) - V^{\expertpolicy}_{R_{\theta}}(s)}{s\sim\rho_E}\\
    0\leq\expectationknowing{V^{\pi^*_{R_{\theta}}}_{R_{\theta}}(s)-V^{\expertpolicy}_{R_{\theta}}(s)}{s\sim\rho_E} &\leq \frac{C_f}{1-\gamma}
    \expectationknowing{\bellmanoptopeval{\rewardfunc_{\theta}}{}V^{\expertpolicy}_{R_{\theta}}(s) - V^{\expertpolicy}_{R_{\theta}}(s)}{s\sim\rho_E}\\
    \label{erooesidual}
  \end{align}
  Ce résultat est similaire à celui de~\citep[Theoreme~4.2]{munos2007performance}. 

  -> dont nous rappelons l'énoncé autoref machin
...
Il nous faut maintenant borner le résidu de Bellman $\expectationknowing{\bellmanoptopeval{\rewardfunc_{\theta}}{}V^{\expertpolicy}_{R_{\theta}}(s) -
  V^{\expertpolicy}_{R_{\theta}}(s)}{s\sim\rho_E}$, que nous décomposons en :
  \begin{equation}
...
\begin{align}
\expectationknowing{\bellmanoptopeval{\rewardfunc_{\theta}}{} V^{\expertpolicy}_{R_{\theta}}(s) - \bellmanevalopeval{\classifpolicy}{\rewardfunc_{\paramirl}}{}V^{\expertpolicy}_{R_{\theta}}(s)}{s\sim\rho_E}\\
\textrm{ et }\expectationknowing{\bellmanevalopeval{\classifpolicy}{\rewardfunc_{\paramirl}}{}V^{\expertpolicy}_{R_{\theta}}(s) - V^{\expertpolicy}_{R_{\theta}}(s)}{s\sim\rho_E}.
\end{align}
