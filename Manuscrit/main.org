...
*** DONE Transformer la distrib de l'expert en mettant le E en indice. Et du coup virer les parenthèses. Pareil pour nimprte quelle distrib
...
* Axe de recherche
Pour toutes définitions raisonnables de /civilisation/ et d'/esclavage/, aucune civilisation n'a jamais existé sans esclavage[fn::En 2012, $3\permil$ de la population mondiale est victime de l'esclavage \citep{ilo2012global}.]. L'esclavage est un système économique extrêmement efficace qui ne sera probablement éradiqué que lorsqu'une solution alternative plus attrayante sera disponible.

L'esclave peut être avantageusement remplacé par un animal lorsque le travail nécessite beaucoup de force et peu d'intelligence ; un animal est moins sujet à la rébellion organisée.

L'automatisation et l'industrialisation ont permis l'apparition de machines plus performantes, plus sûres et moins coûteuses que leurs équivalents biologiques lorsqu'ils existent. Certains systèmes experts parviennent même à des performances supérieures à celles d'humains qualifiés. La main-d'œuvre qualifiée est prête à accepter ces systèmes pour peu qu'on les présente comme des systèmes d'aide à la décision et non comme des remplaçants. Le coût horaire d'un expert humain étant élevé, la marge de manœuvre financière est également plus grande pour les concepteurs des systèmes automatiques. Voir par exemple l'introduction de Watson dans le domaine médical \citep{ibm2013watson,ibm2013watson2}.

Le problème éthique se situe au niveau de la main-d'œuvre non qualifiée. Aujourd'hui, payer $100$ \$ par mois un travailleur chinois assemble des iPods est plus rentable que de concevoir et déployer un système robotique équivalent \citep{mail2006stark}. Les systèmes robotiques actuels ne sont pas assez versatiles et ne peuvent apprendre une tâche sans que le coût du travail d'ingénierie associé ne rende l'opération prohibitive[fn::Ou simplement légèrement moins rentable que l'exploitation humaine, le but et de maximiser les profits, après tout.]. Un moyen de précipiter l'avènement d'une société de l'abondance[fn::Le terme anglais est plus précis : /post-scarcity/.] où l'esclavage n'aurait plus de sens économique[fn::Il y a d'autres avantages \citep{russell1958praise}.] est donc d'imaginer des méthodes de conception de systèmes automatisés intelligents pouvant à peu de frais réaliser une vaste gamme de travaux[fn::A titre personnel, je me demande si une machine passant avec succès le test de Turing\citep{turing2004can} sera un jour titulaire de droits similaires à ceux qui échoient actuellement aux humains \citep{assembly1948universal}. Cela risquerait de poser problème pour l'exploitation industrielle de tels systèmes.].

** Contrôle optimal 
Le problème du contrôle optimal peut être vu comme la recherche du meilleur moyen de réaliser une tâche. L'automatique[fn::En tant que discipline.] permet la conception de systèmes intelligents qui vont dans une certaine mesure d'adapter à leur environnement. Il n'a pas fallu attendre la révolution informatique pour voir apparaître de tels systèmes. Le régulateur à boules de Watt ou le plongeur d'une chasse d'eau sont des systèmes intelligents.

L'intelligence symbolique a fait fantasmer les auteurs de science fiction comme par exemple \citet{asimov1950robot}, mais l'optimisme qui a mené à, puis suivi les conférences de Dartmouth en 1956 était un peu prématuré. Malgré des progrès immenses dans divers champs, par exemple en traitement du langage naturel, les résultats n'ont pas été à la hauteur des attentes.

Nous nous intéressons ici à une autre forme d'intelligence artificielle, l'apprentissage statistique. Plus ou moins bio-inspirées, les approches statistiques permettent à la machine d'apprendre en adaptant son comportement aux données qu'elle analyse. Un des avantages est de pouvoir utiliser un même algorithme sur plusieurs problèmes différents. Il s'agit d'une voie prometteuse pour concevoir des systèmes adaptatifs, capables d'entreprendre différents travaux sans nécessiter d'intervention humaine importante (et donc couteuse).

Décrire la tâche à effectuer nécessite cependant toujours de la part de l'opérateur humain une double expertise : il doit d'une part comprendre la tâche et l'environnement dans lequel évoluera la machine, et d'autre part être en mesure de décrire cette tâche sous une forme que la machine peut comprendre. Observer un agent performant[fn::Par la suite, nous appellerons cet agent l'/expert/.] et en déduire une description de la tâche compréhensible par la machine permettrait de lever la seconde contrainte. Il n'est pas forcément souhaitable de lever la première, cela signifierait qu'une machine pourrait de manière autonome développer une expertise supérieure à l'expertise humaine[fn::Sarah Connor ?]. 
** Trouver la consigne à partir du contrôle 
L'apprentissage statistique permet de concevoir des systèmes intelligents sans avoir à créer un algorithme /ad-hoc/ à chaque tâche. L'opérateur humain doit cependant toujours faire le pont entre le monde et la machine en fournissant aux algorithmes d'apprentissage statistique une description de la tâche que ceux-ci sont en mesure d'exploiter.

Cette formulation des contraintes de la tâche est souvent particulièrement difficile. Un regard objectif sur les avancées récentes en neurosciences \citep{harris2012free}, tend à montrer que les motivations explicites que nous, humains, sommes en mesure de fournir pour justifier nos gestes ou nos décisions ne sont que des justifications /a posteriori/ de phénomènes neurologiquement plus profonds qui échappent à l'analyse sémantique dont les couches conscientes de nos cerveaux sont capables. Il n'existe aucune justification objective à l'hypothèse que nous sommes capables d'une introspection suffisante pour comprendre nos propres motivations.

On peut s'en convaincre en essayant d'expliquer de manière semi-formelle une tâche modérément complexe comme la conduite automobile. Arrivant sur un obstacle apparaissant soudainement, vaut-il mieux piler ou déboiter ? La décision en situation réelle est prise trop rapidement pour engager un processus conscient. Comment alors espérer décrire la tâche autrement qu'en la démontrant[fn::Ce n'est pas pour rien si les programmes de conduite accompagnée ont tant de succès.] ?
*** Intérêt intrinsèque
# <<hier:intrinseque>> 

Découvrir les motivations d'un agent en l'observant a un intérêt qui dépasse la conception de consignes pour systèmes intelligents.

On peut citer en biologie la recherche d'explications à certains comportements animaux \citep{montague1995bee,doya1995novel}, et en économie la découverte des motivations réelles (et non auto déclarées) du comportement humain \citep{sargent1978estimation} afin de pouvoir affiner des hypothèses comme celle de l'agent économique rationnel.

*** Imitation 
# <<hier:general>>
  Sans chercher à extraire une description de la tâche effectuée par un expert, il est possible d'apprendre à partir de démonstrations simplement en copiant les réactions de l'expert, ou en laissant ce dernier corriger les décisions de l'apprenti ou encore en permettant à l'apprenti de demander l'aide de l'expert de manière interactive. Ces méthodes, les méthodes /supervisées/, sont décrites au \autoref{hier-formalisme}, ainsi que les outils mathématiques qui vont nous permettre de décrire le problème que cette thèse espère contribuer à résoudre.

Au \autoref{hier-etatdelart} sont examinées les approches attaquant, comme nous le faisons, le problème de l'imitation sous l'angle de la découverte d'une description de la tâche effectuée par un expert et qui sera par la suite confiée à un agent chargé d'apprendre le contrôle satisfaisant cette description.

Deux revues de la littérature \citep{argall2009survey,schaal2003computational}, outre l'inventaire et la classification des approches d'apprentissage à partir de démonstrations qu'elles proposent, décrivent le cadre dans lequel il est pertinent d'entreprendre ce type d'apprentissage : il faut que l'expert et l'apprenti partagent leur perception de l'environnement et disposent de moyens d'action similaires.
** Contributions
Nos contributions se situent dans le cadre de l'\gls{irl}, plus particulièrement dans la définition de méthodes n'ayant besoin pour fonctionner que de peu d'information concernant le système à contrôler.

Nous espérons ainsi pouvoir obtenir, à partir d'une démonstration de l'expert, une description de la tâche qu'il effectue, d'une manière qui nécessite le moins possible d'intervention humaine, afin de tirer vers le bas les coûts de développements de nouveaux systèmes robotiques.
...
\begin{fullwidth}
\begin{itemize}
\item \bibentry{klein2011batch}
\item \bibentry{klein2011batch1}
\bibentry{klein2011batch2}
\end{itemize}
\end{fullwidth}

Vient ensuite \gls{scirl} (\autoref{hier-scirl}), un nouvel algorithme d'\gls{irl} pouvant tirer parti de \gls{lstdmu} :

\begin{fullwidth}
\begin{itemize}
\item \bibentry{klein2012structured}
\item \bibentry{klein2012structured1}
\item \bibentry{klein2012structured2}
\item \bibentry{klein2012structured3}
\end{itemize}
\end{fullwidth}

Finalement nous décrivons \gls{cascading} (\autoref{hier-cascading}), il s'agit d'un autre algorithme d'\gls{irl} un peu plus souple que \gls{scirl} :
\begin{fullwidth}
\begin{itemize}
\item \bibentry{klein2012cascading}
\item \bibentry{klein2012cascading1}
\end{itemize}
\end{fullwidth}
** Problèmes jouets
...
Le problème du pendule inversé consiste à maintenir en équilibre une masse située au bout d'un bras en commandant les mouvements du chariot à la base du bras. Il s'agit d'un problème continu, la position du pendule étant décrite par l'angle du bras, à valeurs dans $[-\frac{\pi}{2},\frac{\pi}{2}]$ et sans vitesse dans $\reals$. 
...
Ce problème a été utilisé comme banc de test de l'algorithme d'\gls{rl} \gls{lspi}. \citet{lagoudakis2003least} en décrivent la dynamique, basée sur la résolution des équations différentielles du mouvement par la méthode d'Euler.



L'expert pour ce problème est un agent entraîné avec \gls{lspi} ; il parvient à maintenir le pendule en équilibre indéfiniment, mais la simulation s'arrête après $3000$ pas de temps, soit 5 minutes de temps simulé.
...
Pour les espaces continus, une paramétrisation usuelle consiste à paver l'espace de fonctions à base radiale, telles les gaussiennes. On assigne un nombre $\gaussperdim{\dimindex}$ à chacune des dimensions $0 < \dimindex \leq \dimstate$ de l'espace d'état et l'on construit un maillage de $\dimpsi = \prod_{\dimindex=1}^{\dimstate}\gaussperdim{\dimindex}$ points $m_{\dimindexbis}, 0<\dimindexbis\leq\dimpsi$ répartis à équidistance dans l'espace qui seront les centres des $\dimpsi$ composantes gaussiennes de la fonction d'attribut. La variance pour une dimension $\dimindex$ peut être choisie par exemple comme (avec $\component{\state}{\dimindex}$ la $\dimindex^\textrm{ième}$ composante de $\state$) :
\begin{equation}
...
Le deux algorithmes /fitted/-$Q$ et \gls{lspi} sont \gls{offpolicy} et \gls{batch}. Cela signifie respectivement qu'ils peuvent utiliser des données échantillonnées une fois pour toutes et n'ont pas besoin de contrôler le système. Ils sont donc plus efficaces en échantillons que les algorithmes en ligne (mais plus coûteux en temps de calcul) et plus sûrs aussi, puisque l'on a pas à placer un agent encore non entraîné au contrôle du système.
...
# <<hier:recompense>>
*** Espace de départ
En \gls{rl}, on utilise souvent[fn::L'algorithme SARSA utilise comme son nom l'indique une séquence augmentée de l'action $a'$ suivant l'état d'arrivée $s'$.] la transition $\state,\action,\state',r$ comme unité d'information. On y trouve de l'information sur la dynamique du système : l'agent ayant choisi $a$ en $s$, il est arrivé en $s'$, ainsi que le signal de récompense. Il peut être envisagé d'utiliser une récompense dépendant de la transition complète $s,a,s'$, mais on peut facilement définir une récompense parfaitement équivalente sur $\statespace\times \actionspace$ :
\begin{equation}
...
\Sortie{Un vecteur de paramètres $\paramirl$ pour la récompense $\rewardfunc_{\paramirl} = \transpose{\paramirl}\featurestatefunc$}
...
\begin{align}
\|\appr{\rlvalue{\policy}{\rewardfunc}} - \rlvalue{\policy}{\rewardfunc}\|_N &\leq \epsilon_{\phi} + O\left(\sqrt{\frac{\ln(\frac{1}{\delta})}{\nbsamples}}\right) + O\left(\frac{1}{\nbsamples}\right)\\
\textrm{avec :}\|f\|_N &= \frac{1}{\nbsamples}\sum_{\timeindex=1}^{\nbsamples}f(s_{\timeindex})^2
\end{align}
Chaque composante de l'attribut moyen est exactement une fonction de valeur (\autoref{eq:muvaleurvec}) cette borne s'y applique donc directement. La différence entre la valeur recherchée $\rlvalue{\policy}{\component{\featurestateactionfunc_{\rewardfunc}}{\dimindex}}$ et la valeur trouvée par \gls{lstdmu} $\component{\rlmu{\policy}}{\dimindex}$ est donc bornée par le terme d'erreur relatif au biais inductif $\epsilon_\phi$ qui dépend de la différence entre la meilleure approximation de $\rlvalue{\policy}{\component{\featurestateactionfunc_{\rewardfunc}}{\dimindex}}$ dans l'espace d'hypothèse induit par $\featurestateactionfunc_\mu$ et $\rlvalue{\policy}{\component{\featurestateactionfunc_{\rewardfunc}}{\dimindex}}$ elle-même. \`A cela s'ajoutent deux termes dépendant de la taille $\nbsamples$ de la base d'entraînement. L'un en $\frac{1}{\sqrt{\nbsamples}}$, également dépendant de la probabilité $\delta$, l'autre en $\frac{1}{N}$.

En fonction de la norme que l'on choisit, il est possible d'étendre la borne à la fonction vectorielle $\hat{\mu}^\pi_{\zeta}$ renvoyée par \gls{lstdmu} avec une probabilité $1 - \dimx{\phi_R}\delta$ :
\begin{align}
\|\hat{\mu}^\pi_{\zeta} - \rlmu{\policy}\| &\leq \epsilon_{\phi} + O\left(\sqrt{\frac{\ln(\frac{1}{\delta})}{\nbsamples}}\right) + O\left(\frac{1}{\nbsamples}\right),\\
\textrm{avec : } \|\mu\| &= \max_{\dimindex \in \llbracket1,\dimx{\phi_R}\rrbracket} \|\mu\|_N.
\end{align}
...
L'algorithme \gls{lstdmu} que nous proposons nécessite pour fonctionner moins de données que l'algorithme d'\gls{rl} \gls{lspi}. Les garanties théoriques applicables à \gls{lstd} se transfèrent à \gls{lstdmu}. De manière générale, on peut imaginer adapter d'autres résultats de \gls{lstd} à \gls{lstdmu}, comme par exemple \citep{geist2012dantzig,geist2012recursive}.
* \glsentrytext{scirl}
# <<hier:scirl>>  
...
\begin{equation}
\classifscorefunc_{\paramirl} = \transpose{\paramirl}\appr{\expertmu}.
\end{equation}
...
\Donnees{
Une approximation $\appr{\expertmu}$ de l'attribut moyen de l'expert\;
...
\begin{equation}
\appr{\expertmu}(\state_{\datasetindex},\action_{\datasetindex}) = \sum\limits_{\timeindex = 0}^{\muhorizon-1-\datasetindex} \discount^{\timeindex}\featurestateactionfunc_{\rewardfunc}(\state_{\timeindex+\datasetindex},\action_{\timeindex+\datasetindex}).
\label{eq:montecarlodupauvre}
\end{equation}
Il faut noter que la qualité de l'estimation risque de diminuer à mesure que l'on s'approche de la fin de la trajectoire, puisque le nombre d'échantillons participant au calcul diminue. Cette méthode fonctionne bien sur les problèmes quasi-déterministes mais risque de poser problème sur les dynamiques plus stochastiques. Utiliser \gls{lstdmu} est la solution la plus souple. Si seules les données de l'expert sont disponibles, une version \gls{onpolicy} de l'algorithme permettra d'obtenir une estimation $\appr{\expertmu}$ sur les couples $(s_i,a_i)$ présents dans la base d'entraînement. Si des données non expertes sont également disponibles, alors \gls{lstdmu} dans sa version \gls{offpolicy} saura les exploiter.
...
\begin{equation}
\appr{\expertmu}(\state_i,\action \neq \action) = \discount \appr{\expertmu}(\state_i,a_i)
...
Procéder à la descente de sous-gradient dont le sous-gradient est :
\begin{equation*}
\subgrad\structuredcost(\paramirl) = \sum_{\datasetindex=1}^{N}\appr{\expertmu}\left(\state_{\datasetindex},\arg\max_{\action\in\actionspace}\transpose{\paramirl}\appr{\expertmu}(\state_{\datasetindex},\action) + \margin(\state_{\datasetindex},\action)\right) - \appr{\expertmu}(\state_{\datasetindex},\action_{\datasetindex})\;
\end{equation*}
\Retour{$\paramirl$}
...
Maintenant que le principe de \gls{scirl} ainsi qu'une instanciation particulière utilisant l'\autoref{alg:taskar} et \gls{lstdmu} sont en place, intéressons-nous aux garanties théoriques qu'il est possible de fournir sur les performances d'imitation de \gls{scirl}.
...
\begin{theorem}[Borne des performances de \glstext{scirl}]
  ...
  \end{equation}
\end{theorem}
La preuve, due au travail de Matthieu Geist[fn::Co-auteur des contributions concernant \gls{scirl} \citep{klein2012structured,klein2012structured1,klein2012structured2,klein2012structured3}.] est reportée en \autoref{hier-demo}.
...
# <<hier:expmountaincar>>
    ...
    Le classifieur dont les performances sont illustrées est une \gls{svm} à noyau gaussien dont les paramètres sont choisis automatiquement par l'implémentation utilisée[fn::[[http://scikit-learn.org/stable/modules/generated/sklearn.svm.SVC.html][Module SVM de Scikit-learn]]]. Les performances de cette technique étant très légèrement supérieures à ceux du classifieur structuré de \citet{taskar2005learning} (\autoref{alg:taskar}), ce sont celles que nous affichons ici.

    En ordonnée sur la \autoref{fig:scirlmountaincar}, nous représentons le nombre de pas nécessaire à l'agent pour qu'il atteigne le but (plafonné à $300$), lorsque l'état de départ est tiré aléatoirement dans l'espace d'état entier. Les valeurs affichées sont la moyenne et l'écart-type sur $100$ expériences. Ce critère est directement lié à la valeur de la politique pour la récompense inconnue $\expertreward$ : si l'expert touche une récompense de $1$ lorsqu'il atteint l'objectif, et que la récompense est nulle partout ailleurs, du fait de l'amortissement temporel la valeur de sa politique en un état vaut $\discount^T$ où $T$ est le nombre de pas qui le sépare de l'objectif. En conséquence, ce que nous traçons est le logarithme en base $\discount$ de la valeur moyenne de la politique.
...
\caption[Algorithmes d'imitation sur le \glstext{highway}]{Algorithmes d'imitation sur le \glstext{highway}. En plus de la valeur de l'expert comme dans la \autoref{fig:scirlhighway}, la valeur d'une politique apprise sur une récompense aléatoire donne un point de référence de valeur minimale. On montre ici l'écart-type et la moyenne sur $100$ expériences.}
...
** Conclusion
    ...
    \gls{scirl} en particulier a pu fonctionner à partir des mêmes données que la méthode supervisée, sans données supplémentaires comme \gls{relent} en nécessite. Il a su exploiter les informations contenues dans les transitions pour introduire la dynamique du système dans la résolution du problème de l'imitation. L'heuristique que nous avons présentée \autoref{eq:heuristiquescirl} et que la validation théorique ne permet pas facilement d'analyser a fait ses preuves sur des problèmes déterministes. \gls{scirl} est moins gourmand en temps de calcul et en données qu'un algorithme d'\gls{rl} comme par exemple \gls{lspi}. Il faut tenir compte de cela, puisque contrairement aux méthodes supervisées qui renvoient directement une politique de contrôle, \gls{scirl} renvoie une récompense qu'il faut optimiser, en utilisant par exemple un algorithme d'\gls{rl} auquel il faut fournir de l'information dont \gls{scirl} n'a pas besoin. Le gain en performance justifie probablement l'effort lié à la résolution du \gls{mdp}, des méthodes /ad-hoc/ permettant de traiter les problèmes qui ne manqueront pas de se poser.
...
Le terme $\Delta_q$ cache un facteur en $\frac{1}{1-\discount}$, on retrouve le même type de constantes que dans le \autoref{thm:scirl}.
...
Les deux termes d'erreurs principaux présents dans cette borne, $\epsilon_C$ et $\epsilon_R$ sont conformes aux termes d'erreur usuels en apprentissage supervisé. Utiliser les nombreuses astuces et adaptations rendues possibles par l'usage d'un classifieur et d'un régresseur de l'état de l'art permet d'espérer minimiser ces deux termes. Les résultats théoriques de l'apprentissage supervisé peuvent être facilement intégrés à \gls{cascading}. Il est plus raisonnable d'espérer contrôler le terme $\epsilon_R$ présent dans cette borne que le terme $\bar\epsilon_Q$ de la borne du \autoref{thm:scirl}.
...
Nous retrouvons ici les bancs d'essais utilisés \autoref{hier-scirlexp}, le \gls{mountaincar} et le \gls{highway}. La classification s'étant avérée moins performante que les méthodes d'\gls{irl}, nous nous concentrons ici sur celles-ci, et comparons \gls{scirl}, \gls{cascading} et \gls{relent}. Le critère utilisé est le même que celui des expériences précédentes, à savoir un critère donnant une idée de l'optimalité de la politique optimale pour $\apprRc$ par rapport à la récompense inconnue $\expertreward$.
...
*** \glstext{cascading} sur le mountain car
Le protocole expérimental est le même que dans la \autoref{hier-expmountaincar}. Le classifieur choisi pour \gls{cascading} est le même que celui dont les performances sont illustrées \autoref{fig:csimountaincar}. Il s'agit de l'implémentation d'une SVM[fn::[[http://scikit-learn.org/stable/modules/generated/sklearn.svm.SVC.html][Module SVM de Scikit-learn]]]. Le noyau est gaussien, les paramètres par défaut sont utilisés. Nous sommes dans un cas où l'ingénierie déployée par l'opérateur est minimale. Le régresseur est choisi dans le même esprit, il s'agit d'une SVR[fn::[[http://scikit-learn.org/stable/modules/generated/sklearn.svm.SVR.html][Module SVR de Scikit-learn]]] utilisée avec ses paramètres par défaut. 
...
\end{table}
*** \glstext{cascading} sur le \glstext{highway}
...
** Conclusion 
   ...
   La souplesse offerte par la liberté de choix des deux approches supervisées laisse à l'opérateur la possibilité d'adapter l'approche au problème qu'il cherche à résoudre. Une heuristique est offerte qui permet à \gls{cascading} de fonctionner avec une expertise humaine minimale en n'utilisant que des données expertes. Cela est démontré sur les mêmes bancs d'essais que ceux qui ont permis la validation empirique de \gls{scirl}. Les performances empiriques offertes par \gls{cascading} sont légèrement supérieures à celles de \gls{scirl}. Les deux approches sont plus performantes que l'approche supervisée au prix cependant de devoir résoudre le \gls{mdp} une fois la récompense obtenue.

* Rappel des contributions 
L'un des freins à l'adoption généralisée de la robotique dans la société est le coût élevé des dispositifs automatiques. Du fait de l'augmentation exponentielle des capacités techniques du matériel, le goulot d'étranglement se situe de plus en plus au niveau du coût de conception des systèmes intelligents. Le développement d'algorithmes génériques pouvant s'adapter à une vaste gamme de problèmes est l'un des moyens pour lesquels la réduction de ces coûts de développement pourrait être réalisée.
...
Le cadre de l'\gls{irl} permet de découvrir une récompense cohérente avec le comportement d'un expert. Nous proposons deux nouvelles approches d'\gls{irl} (\gls{cascading} et \gls{scirl}) que nous comparons à une méthode existante, ainsi qu'à des algorithmes d'imitation supervisée, qui n'utilisent pas la notion de récompense et se contentent de copier directement l'expert. L'un de nos nouveaux algorithmes d'\gls{irl} utilise une estimation de l'attribut moyen de l'expert pouvant être obtenue grâce à une autre de nos contributions, \gls{lstdmu}.

Nous démontrons empiriquement que les algorithmes d'\gls{irl} que nous proposons peuvent fonctionner avec peu de données. Bien que les problèmes sur lesquels nous les testons soient des problèmes jouets, nos algorithmes sont à notre connaissance les seuls algorithmes d'\gls{irl} pouvant fonctionner uniquement avec des données expertes[fn::moyennant l'utilisation d'heuristiques]. Les résultats obtenus montrent des performances supérieures à celles des algorithmes supervisés ; il faut cependant trouver moyen d'optimiser la récompense pour trouver une politique tandis que les algorithmes supervisés la fournissent directement.\\

Les systèmes réels sont fragiles et interagir avec est coûteux. Il est également difficile de les simuler. On comprend alors l'intérêt de développer des algorithmes moins gourmands en données.
...
Il serait souhaitable de renforcer l'étude empirique des algorithmes que nous avons proposés en les appliquant à d'autres problèmes. La difficulté n'est pas dans l'application des algorithmes d'\gls{irl} eux même, mais dans l'optimisation de la récompense qu'ils trouvent afin d'obtenir une politique de contrôle.

Pour palier à ce problème[fn::Qui n'affecte pas les approches supervisées.], nous pourrions également tenter de développer des algorithmes d'\gls{irl} pour lesquels l'optimisation finale de la récompense est simplifiée ou contournée.

Finalement, il faudrait intégrer l'\gls{rl} et l'\gls{irl} dans des systèmes informatiques réels aux mains d'utilisateurs non qualifiés.
\appendix
* Démonstrations
# <<hier:demo>>
...
\Rightarrow (I-\transprobmat{\pi^*_{R_{\theta}}})(V^{\pi^*_{R_{\theta}}}_{R_{\theta}} - V^{\expertpolicy}_{R_{\theta}})&=\bellmanoptopeval{\rewardfunc_{\theta}}{} V^{\expertpolicy}_{R_{\theta}} - V^{\expertpolicy}_{R_{\theta}}
\end{align}
...
Il nous faut maintenant borner le résidu de Bellman $\expectationknowing{\bellmanoptopeval{\rewardfunc_{\theta}}{}V^{\expertpolicy}_{R_{\theta}}(s) -
  V^{\expertpolicy}_{R_{\theta}}(s)}{s\sim\expertdistrib}$, que nous décomposons en :
  \begin{equation}
...
\begin{align}
\expectationknowing{\bellmanoptopeval{\rewardfunc_{\theta}}{} V^{\expertpolicy}_{R_{\theta}}(s) - \bellmanevalopeval{\classifpolicy}{\rewardfunc_{\paramirl}}{}V^{\expertpolicy}_{R_{\theta}}(s)}{s\sim\expertdistrib}\\
\textrm{ et }\expectationknowing{\bellmanevalopeval{\classifpolicy}{\rewardfunc_{\paramirl}}{}V^{\expertpolicy}_{R_{\theta}}(s) - V^{\expertpolicy}_{R_{\theta}}(s)}{s\sim\expertdistrib}.
\end{align}
  ...
  \end{align}
  D'après l'inégalité de Hoelder[fn::Qui stipule que si $1\leq p,q\leq\infty,\frac{1}{p}+\frac{1}{q} = 1$, alors \begin{equation*}\|fg\|_1\leq \|f\|_p\|g\|_q.\end{equation*}] appliquée ici avec $p=1$ et $q=\infty$ :
\begin{align}
...
\bibliography{../../Biblio/Biblio,../../Biblio/BiblioKlein.bib}
