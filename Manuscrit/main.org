...
\begin{eqnarray}
\label{eq:Vdefsum}
\rlvalue{\policy}{\rewardfunc}(\state_{0})&=&\sum\limits_{\timeindex = 0}^{\timehorizon} \discount^{\timeindex}\staterewardfunceval{\state_{\timeindex}}\\
...
\begin{equation}
\weightedexpectation{\state}{\expertdistribution}{\rlvalue{\expertpolicy}{\expertreward}(\state) - \rlvalue{\optimalpolicy{\appr{\rewardfunc}}}{\expertreward}(\state)},
\end{equation}
l'autre est calculable :
\begin{equation}
\weightedexpectation{\state}{\spacedistrib}{\rlvalue{\optimalpolicy{\appr{\rewardfunc}}}{\appr{\rewardfunc}}(\state) - \rlvalue{\expertpolicy}{\appr{\rewardfunc}}(\state)}.
\end{equation}
...
\label{eq:mudef}
\textrm{avec }\forall \timeindex \in \llbracket 1;T\rrbracket, \state_{\timeindex} &\sim& \transprobfunceval{\state_{\timeindex-1}}{\policy(\state_{\timeindex-1})}{\cdot},\\
\rlvalue{\policy}{\appr{\rewardfunc}}(\state_{0})&=&\sum\limits_{\timeindex = 0}^{\timehorizon} \discount^{\timeindex}\appr{\rewardfunc}(\state_{\timeindex})\\
&=&\sum\limits_{\timeindex = 0}^{\timehorizon} \discount^{\timeindex}\transpose{\paramirl}\featurestatefunc(\state_{\timeindex})\\
&=&\transpose{\paramirl}\sum\limits_{\timeindex = 0}^{\timehorizon} \discount^{\timeindex}\featurestatefunc(\state_{\timeindex})\\
&=&\transpose{\paramirl}\rlmu{\policy}(\state_0)
\end{eqnarray}
...
\begin{eqnarray}
\textrm{avec }\forall \timeindex \in \llbracket 1;T\rrbracket, \state_{\timeindex} &\sim& \transprobfunceval{\state_{\timeindex-1}}{\policy(\state_{\timeindex-1})}{\cdot},\\
\rlmu{\policy}(\state_{0})&=&\sum\limits_{\timeindex = 0}^{\timehorizon} \discount^{\timeindex}\featurestatefunc(\state_{\timeindex})\\
...
\rlmu{\policy}(\state_{0},\action)&=&\sum\limits_{\timeindex = 0}^{\timehorizon} \discount^{\timeindex}\featurestatefunc(\state_{\timeindex}).
\end{eqnarray}
...
\begin{equation}
\rlmu{\policy}(\state_{0})=\sum\limits_{\timeindex = 0}^{\timehorizon} \discount^{\timeindex}\featurestateactionfunc\left(\state_{\timeindex},\policy(\state_{\timeindex})\right).
\end{equation}
# illustration mountain car : la forme en escargot est caractéristique elle correspond à des va-et-vient dont l'apogée est de plus en plus haute.
