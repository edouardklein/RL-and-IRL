...
\end{equation}
L'algorithme \gls{lstdmu} du \autoref{hier-lstdmu} est tout indiqué pour fournir cette estimation. Si celle-ci est parfaite, alors $\classifscorefunc_{\paramirl}$ est exactement la fonction de qualité de l'expert pour la récompense $R_\theta$, $\quality{\expertpolicy}{\rewardfunc_{\paramirl}}$ (\autoref{eq:qemuparam}).
