...
# snippet: La notion d'agent artificiel déborde sur l'espace d'état, qui n'est lui même du coup pas directement lié à l'environnement. Un agent, ce n'est pas seulement une politique, mais aussi la définition de l'espace d'état et d'action, qui ne sont qu'une vue (plus ou moins bonne selon l'ingénierie) de la réalité
   ...
   Les espaces d'état et d'action ne sont généralement pas des espaces vectoriels : la plage de valeurs que peut prendre une composante est rarement illimitée. Il s'agit même parfois d'ensembles finis. Dans ce cas, on s'autorisera à manipuler les états comme les $|\statespace|$ premiers entiers, il existe en effet un isomorphisme entre $\statespace$ et $\llbracket 1;|\statespace|\rrbracket$. C'est dans les problèmes qui nous intéressent quasiment toujours le cas pour l'espace d'action. Nous considérons par défaut qu'il s'agit d'un ensemble fini de faible cardinalité. La plupart des notations que nous utiliserons supposent un espace d'état également fini, l'usage d'attributs judicieusement choisis (\autoref{hier-attributs}) permet cependant dans la plupart des cas l'extension pratique au cas continu.
...
# ?s expertpolicy '\pi^E' 'Politique de l{\apos}expert'
   ...
   Bien souvent, cependant, il est impossible de connaître intégralement la politique de l'expert (une cause habituelle est un trop grand espace d'état). Il faut alors se contenter de $\nbsamples$ exemples[fn::L'indice $sa$ de $\satrace{\expertpolicy}$ indique quels éléments (ici des couples $(\state_{\datasetindex},\action_{\datasetindex})$) composent la base. A l'\autoref{eq:sastracedef}, par exemple, ce sont des quadruplets $(\state_i,a_i,s'_i,r_i)$ qui sont utilisés.]:
...
\end{equation}
   ...
   Le problème de l'imitation se trouve ainsi réduit à celui de la classification. Étant donné que l'espace d'action est fini et de faible cardinalité, chaque action est vue comme une étiquette à appliquer à un état. La démonstration de l'expert fournit la base d'entraînement.
...
C'est l'équation d'évaluation de \citet{bellman1956dynamic} qui est à l'origine de l'opérateur d'évaluation de Bellman :
\begin{align}
...
Il existe plusieurs schémas d'\gls{rl}. Certains comme par exemple $Q$-learning \citep{watkins1989learning} adaptent le principe d'itération de la valeur \citep{bellman1956dynamic}. Une amélioration permettant le fonctionnement hors ligne, à partir d'une base de données acquise une fois pour toutes est présentée par \citet{gordon1995stable}, c'est l'algorithme /fitted/-$Q$, que nous exposons \autoref{alg:fittedq}. Le choix du régresseur de l'étape de la \autoref{alg:fittedq:regression} étant laissé libre, /fitted/-$Q$ a donné lieu à de nombreux travaux exploitant cette liberté. Citons par exemple l'utilisation de réseaux de neurones par \citet{riedmiller2005neuralQ} ou d'arbres par \citet{ernst2005tree}.
...
Sachant tout cela, nous choisissons un classifieur à fonction de score $\classifscorefunc_{\paramirl}$ qui puisse être paramétrée linéairement. Comme nous avons identifié le rôle de la fonction de score et celui de la fonction de qualité optimale, nous utiliserions idéalement $\quality{\expertpolicy}{\rewardfunc_{\paramirl}}$ comme fonction de score (dont nous apprendrions les paramètres $\theta$, ce qui définit indirectement la fonction de récompense). Toutefois $\mu^E$ n'est généralement pas connue, nous utilisons comme attribut pour la paramétrisation linéaire de la fonction de score une approximation $\hat\mu^E$ de l'attribut moyen de l'expert $\expertmu$ afin d'obtenir une expression identique à l'\autoref{eq:qemuparam} :
\begin{equation}
\classifscorefunc_{\paramirl} = \transpose{\paramirl}\hat{\mu}^E.
\end{equation}
L'algorithme \gls{lstdmu} du \autoref{hier-lstdmu} est tout indiqué pour fournir cette estimation (rappelons que si celle-ci est parfaite, alors $\classifscorefunc_{\paramirl}$ est exactement la fonction de qualité de l'expert pour la récompense $R_\theta$, c'est-à-dire  $\quality{\expertpolicy}{\rewardfunc_{\paramirl}}$ (\autoref{eq:qemuparam})).
...
Cette méthode a pour caractéristique commune avec les travaux de \citet{ratliff2006maximum} d'utiliser le classifieur à marge structurée de \citet{taskar2005learning}. Elle en diffère cependant fondamentalement par le niveau auquel le problème de l'\gls{irl} est condidéré. L'algorithme \gls{mmp} proposé dans \citep{ratliff2006maximum} associe à des \gls{mdp} (qui constituent les entrées) des politiques optimales (qui constituent les étiquettes). Cela permet de prendre la structure du \gls{mdp} en compte, mais implique de résoudre les \gls{mdp} en jeu. \`A l'inverse, \gls{scirl} associe des états (les entrées) à des actions (les étiquettes). Il se rapproche ce faisant des travaux présentés dans \citep{ratliff2007imitation}, mais ceux-ci ne faisant pas usage de l'attribut moyen de l'expert $\expertmu$, ils constituent en fait une méthode d'imitation supervisée, tandis que \gls{scirl} prend en compte la structure temporelle du \gls{mdp}.
...
# <<hier:scirlcomplex>>
Le schéma général de \gls{scirl} fourni \autoref{alg:scirlgenerique} n'est pas assez précis pour permettre une analyse de sa complexité algorithmique, qui dépendra du choix de la méthode d'approximation de $\expertmu$ et de la méthode de classification. Plus subtilement, certains détails d'implémentation autorisent un compromis entre mémoire et temps de calcul. En effet il peut être intéressant de stocker les valeurs de $\hat\mu^E$ pour les couples $(s,a)$ dont le classifieur a besoin, plutôt que d'intéroger systématiquement la méthode d'approximation.

# ?s iterlimit 'T' 'Nombre maximum d{\apos}itérations'
Considérons à titre d'exemple le classifieur utilisé dans notre instanciation particulière de \gls{scirl} (\autoref{alg:scirlonlyexpert}). Si l'on cherche à minimiser le temps de calcul, alors il est possible de stocker les valeurs de $\hat \mu^E$ dans une table de hachage de taille $O(\card{\actionspace}\nbsamples\dimx{\phi_R})$ dont nous supposons qu'elle offre un temps d'accès en moyenne constant. La complexité algorithmique de la descente de gradient (pour peu qu'elle soit limitée à $\iterlimit$ itérations) est alors en $O(TN\card{\actionspace}\dimx{\phi_R})$.

Si les valeurs approchées de $\hat\mu^E$ sont trop nombreuses pour être stockées, alors il est possible en utilisant par exemple \gls{lstdmu} de ne stocker que la matrice de taille $\dimx{\phi_\mu}\times\dimx{\phi_R}$ que celui-ci utilise, mais évaluer $\hat\mu^E$ ne se fera alors plus à temps constant, mais en $O(\dimx{\phi_\mu}\dimx{\phi_R})$ (le prix du produit matriciel de l'\autoref{eq:paramlstdmu}). La complexité temporelle de la descente de gradient est alors en $O(TN\card{\actionspace}\dimx{\phi_\mu}(\dimx{\phi_R})^2)$. La table 5.1 regroupe ces résultats.
...
&Temps & Mémoire\\
\midrule
Optimisation temporelle &$O(TN\card{\actionspace}\dimx{\phi_R})$&$O(\card{\actionspace}\nbsamples\dimx{\phi_R})$\\
Optimisation spatiale &$O(TN\card{\actionspace}\dimx{\phi_\mu}(\dimx{\phi_R})^2)$&$O(\dimx{\phi_\mu}\dimx{\phi_R})$\\
\bottomrule
\end{tabular}
\caption[Complexité algorithmique de \glstext{scirl}]{Complexités algorithmiques asymptotiques d'une implémentation de \gls{scirl} faisant usage du classifieur décrit \autoref{hier-taskar}, selon que l'on calcule les valeurs de $\hat\mu^E$ (optimisation spatiale) ou qu'elle sont stockées (optimisation temporelle).}
\label{tab:complexscirl}
\end{table}
...
\end{equation}
\end{proof}
Ce que nous démontrons ici n'est pas simplement l'optimalité de $\pi^E$ pour la récompense $R_theta$, mais le résultat moins trivial concernant l'unicité de $\pi^E$ en tant que politique optimale pour $R_\theta$.

\begin{corollary}[Résistance de \glstext{scirl} aux solutions dégénérées]
La récompense renvoyée par \gls{scirl} ne devrait pas être la récompense nulle (qui est une solution dégénérée au problème de l'\gls{irl}) bien que celle-ci minimise le critère utilisé dans la borne : pour $\paramirl=0$, $\expertpolicy$ est optimale pour $\rewardfunc_{\paramirl}$, de même que n'importe quelle autre politique.
\end{corollary}
\begin{proof}
Avoir $\theta=0$ implique que la politique de classification $\classifpolicy$ est la politique aléatoire uniforme, atteignant l'erreur de classification maximale $\classiferror{\expertdistrib} = \frac{\card{\actionspace}-1}{\card{\actionspace}}$. Le classifieur cherchant à minimiser $\classiferror{\expertdistrib}$, il est peu probable que ce cas advienne.
\end{proof}
** Validation empirique
# <<hier:scirlexp>>
...
*** Complexité algorithmique
Le schéma général de \gls{cascading} laisse comme celui de \gls{scirl} une latitude certaine à l'utilisateur en ce qui concerne le choix des sous routines. Les deux étapes en jeu sont cependant bien plus indépendantes que dans \gls{scirl}. L'interface entre les deux est la construction de la base de données $\rctrace$ entre la classification et la régression.

La complexité algorithmique finale de \gls{cascading} est donc celle de l'étape (classification ou régression) la plus gourmande. Cela dépend donc complètement du type de classifieur et du type de régresseur choisis ainsi que de l'implémentation utilisée (par exemple \citep{chang2011libsvm} ou \citep{lauer2011msvmpack} pour les \gls{svm}).

Si nous prenons l'exemple de l'instanciation de \gls{cascading} utilisée lors de nos expériences de la \autoref{hier-csihighway}, nous voyons que le classifieur (l'algorithme de classification structurée de \citet{taskar2005learning}, \autoref{alg:taskar}) s'exécutera en $O(T\card{\actionspace}\nbsamples\dimx{\phi_R})$ pas de temps, avec comme dans la \autoref{hier-scirlcomplex}, $N$ la taille de la base de données et $T$ le nombre d'itérations maximum pour la descente de gradient. Pour le régresseur (une approche au moindre carrés, \autoref{alg:lsr}) l'opération la plus coûteuse est une inversion de matrice de taille $\dimx{\phi_R}$, qui se fera donc en $O((\dimx{\phi_R})^3)$ pas de temps. La taille de la base de données intervient également, mais uniquement de manière linéaire. La complexité algorithmique totale de \gls{cascading} est donc en $O(N+(\dimx{\phi_R})^3)$. Expérimentalement, \gls{cascading} fonctionnant fort bien avec $N<\dimx{\phi_R}$, l'on retiendra une complexité en $O((\dimx{\phi_R})^3)$.
...
Les deux nouveaux algorithmes d'\gls{irl} que nous proposons disposent de garanties théoriques et une étude empirique illustre le fait qu'ils sont plus performants que les méthodes de l'état de l'art. \`A notre connaissance aucune méthode d'\gls{irl} actuelle n'est en mesure de fonctionner dans le cadre extrêmement contraint dans lequel nous avons testé nos approches, où la politique de l'expert et la dynamique du système ne sont connues qu'à travers une base de données recueillies par l'expert[fn::Rappelons que \gls{relent} dispose d'échantillons recueillis par une politique aléatoire, tandis que \gls{scirl} et \gls{cascading} utilisent chacun une heuristique qui leur permettent de fonctionner en n'utilisant que les données de l'expert.].

Il serait souhaitable de poursuivre cette étude empirique selon deux axes. Tout d'abord il faudrait appliquer nos algorithmes à des problèmes déjà résolus par d'autres méthodes, à des fins comparatives et dans le but de mieux comprendre leur comportement. Cela permettrait notamment d'en cerner les limites et les conditions idéales d'application, ouvrant la voie à des avancées selon le second axe qui consiste à appliquer nos méthodes à des problèmes concrets où l'utilisation d'algorithmes d'apprentissage statistique n'a jusqu'à présent pas été tentée ou n'a pas donnée de bons résultats.

L'étude du comportement empirique de \gls{scirl} et \gls{cascading} doit non seulement être menée sur des problèmes plus nombreux et plus divers que ceux utilisés ici, mais doit également mettre en jeu plusieurs implémentations de nos algorithmes d'\gls{irl}, différants par le choix des sous-routines. En effet, les garanties théoriques proposées (qui ne prennent pas l'heuristique en compte mais supposent un cadre peu contraint, où des données échantillonnant tout le \gls{mdp} et non la seule politique experte sont disponibles) ne présupposent pas l'emploi d'un classifieur à fonction de score (linéaire pour \gls{scirl}), d'un schéma d'estimation de l'attribut moyen (pour \gls{scirl}) ou d'un régresseur (pour \gls{cascading}) particulier, mais traitent de la propagation des erreurs induites par ces éléments. En pratique, une vaste gamme de techniques d'apprentissage supervisé peut être utilisée (particulièrement pour \gls{cascading}) pour limiter ces erreurs. Il serait intéressant de découvrir dans quelle mesure cette marge de manœuvre permet en pratique à nos algorithmes de s'adapter au problème qui leur est soumis.

Deux domaines correspondant à chacun des deux axes de développement de l'étude empirique de nos algorithmes d'\gls{irl} ont commencé à être explorés, mais les travaux n'ont pu être menés à terme et de fait ne sont que mentionnés dans ce manuscrit. Le premier est l'/Arcade Learning Environment/ (ALE) : il s'agit d'un émulateur d'Atari 2600 permettant de jouer à une vaste collection de jeux vidéos. Il est d'ores et déjà possible d'appliquer des algorithmes d'\gls{rl} afin d'apprendre une politique de contrôle maximisant le score \citep{bellemare12arcade}. Il reste à régler des problèmes techniques relatifs au remplacement de ce score par la récompense issue de l'exécution d'un algorithme d'\gls{irl}. Le second est l'application d'algorithmes d'\gls{irl} à la commande d'un bras robot à partir de signaux issus d'une interface cerveau/machine (/Brain Computer Interface/, BCI)[fn::Voir la \autoref{hier-autrestravaux} pour une description des travaux effectués sur la BCI.].  Dans ces deux cas, la difficulté n'est pas dans l'application des algorithmes d'\gls{irl} eux-même, mais dans l'optimisation de la récompense qu'ils trouvent afin d'obtenir une politique de contrôle. Nous avons pour les travaux exposés dans ce manuscrit considéré que cette étape ne posait pas problème, afin de nous concentrer sur le problème de l'\gls{irl} tel qu'envisagé par la communauté. L'application à des problèmes plus complexes que ceux qui constituent notre étude empirique doit faire face à l'optimisation de la récompense, il faut alors faire appel à des méthodes de l'état de l'art en \gls{rl} et des adaptations /ad-hoc/ au problème (par exemple dans le choix des attributs).
...
La découverte d'une fonction de récompense cohérente avec le comportement de l'expert reste cependant un problème intéressant en soi. Lorsque les attributs $\phi_R$ en jeu dans l'approximation linéaire de la récompense ont une valeur sémantique, il est possible d'utiliser l'\gls{irl} pour découvrir et analyser les motivations d'un agent. Les nouveaux algorithmes d'\gls{irl} que nous proposons pourraient donc se montrer utiles pour poursuivre des travaux de ce type entrepris en économie \citep{sargent1978estimation,rust1994people} ou en biologie \citep{montague1995bee,doya1995novel}. Au delà de l'intérêt sémantique de la récompense des travaux très récents \citep{piot2013learning} exhibent une contradiction entre intuition théorique et résultats pratiques. Selon les résultats théoriques l'étape d'optimisation approximative de la récompense introduit des erreurs qui pénalisent la performance de l'imitation par le biais de l'\gls{irl} par rapport à l'imitation supervisée. Empiriquement, trouver une récompense puis l'optimiser peut permettre de meilleures performances en imitation que les méthodes supervisées, particulièrement si la récompense est peu informative. Dans le cas d'un changement de dynamique entre le moment où l'expert effectue sa démonstration et celui où l'agent le remplace (cas qui sort du cadre de l'étude théorique), l'avantage de l'\gls{irl} devient encore plus clair, l'optimisation de la fonction de récompense permettant de s'adapter à la nouvelle dynamique.

\`A plus long terme, la similitude entre fonction de score d'un classifieur et fonction de qualité d'un agent en \gls{rl}, qui est à la base de nos approches d'\gls{irl} \gls{scirl} et \gls{cascading}, pourrait être exploitée dans le cadre de la classification mutli-classe semi-supervisée \citep{chapelle2006semi}. Cela permettrait soit de créer de nouveaux algorithmes, soit d'adapter les algorithmes existants à des cas où il existe une structure implicite dans les données à disposition, par exemple si la trace de l'expert est constituée d'une suite d'états, sans information explicite quant aux actions qu'il a choisies. Cela correspond notamment au cas où il est impossible d'observer l'expert "de l'intérieur" pour des raisons techniques[fn::Par exemple en biologie si l'expert est une hirondelle africaine.].
