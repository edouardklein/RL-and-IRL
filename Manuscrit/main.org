...
Finalement nous présentons notre deuxième algorithme d'\gls{irl}, \gls{cascading}, au \autoref{hier-cascading}. Nous reproduisons les résultats théoriques obtenus par Bilal Piot, chronologiquement antérieurs à ceux que nous fournissons pour \gls{scirl}. Nous offrons une autre heuristique qui permet à \gls{cascading} de fonctionner comme \gls{scirl} en utilisant uniquement des données issues de la démonstration de l'expert. Nous montrons qu'expérimentalement les performances de \gls{cascading} (dans sa version heuristique) sont légèrement supérieures à celles de \gls{scirl}. 
