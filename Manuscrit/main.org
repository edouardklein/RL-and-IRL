...
# ?s ccoeff 'C_f' 'Coefficient de concentration'
...
\begin{proof}[Preuve du Théorème \ref{th:scirl}]
...
# <<hier:expmountaincar>>
    ...
    Le classifieur dont les performances sont illustrées est une \gls{svm} à noyau gaussien dont les paramètres sont choisis automatiquement par l'implémentation utilisée. Les performances de cette technique étant très légèrement supérieures à ceux du classifieur structuré de \citet{taskar2005learning}, ce sont celles qui nous affichons ici.
...
Les avantages présentés par \gls{scirl} découlent de l'identification de la fonction de score d'un classifieur à la fonction de qualité de l'expert. Cela permet d'introduire la dynamique du système dans une approche supervisée, combinant les avantages en terme de généralisation caractéristiques des approches d'\gls{irl} et la simplicité d'utilisation des approches sueprvisées.
...
\begin{equation}
\Rc(\state,\action) = \classifscorefunc(\state,\action) - \discount \sum_{\state'\in \statespace}\transprobfunceval{\state}{\action}{\state'} \classifscorefunc(\state',\expertpolicy(\state')).
\end{equation}


    ->Avec un trace experte on apprend un fonction de score (ie avec nimporte quel classifieur du marché)

    ->Avec une trace quelconque on échantillonne l'équation de Bellman inversée

    ->Algo général

    ->Tout cela est bien beau, mais que se passe-t-il quand on a que les données expertes. Heuristique mon cher.
    
    -> L'heuristique est semblables à la justification de la plupart des algos comme par exemple la marge l de la l'irl/classif, où l'expert doit être meilleur que les autres. C'est une affirmation assez naturelle dans le domaine pour essayer d'éviter les solutions dégénérées.

    ->Algo heuristique
** Validation théorique 
# <<hier:csitheo>>
Snippet: pour les objets, le coeff de concentration est plus fin que le truc de bourrin de scirl

Recopier le théorème du papier

Recopier la preuve du papier

Expliquer les différences avec l'analyse de scirl

Dire que les termes sont plus facile à manipuler puisque tout est standard

Corollaire intéressant

** Validation pratique rapide (problèmes jouets)
# <<hier:csiempirique>
-> Encore une fois, l'heuristique n'est pas prise en compte dans l'analyse

-> Il faut donc la tester empiriquement

-> On réutilise les benchmarks de scirl, puisqu'ils ont du sens, on pourra comparer à SCIRL et RelEnt, dont on ne change pas les 

-> On utilise l'instanciation heuristique

*** mountain car
-> Même classification que le classifieur de l'expérience sur scirl (ref et url, rajouter l'url là haut)

-> SVR pour la régression (ref et url)

-> Algo complet

-> On rappelle la constitution des données, idem que pour scirl

-> figure sans classif

-> p-valeurs

-> CSI est meilleur pour le premier point et se confond avec SCIRL ensuite, cf les p-valeurs

*** Highway
-> Classification structurée, pareil que sur scirl

-> Least square pour la régression

-> Algo complet

-> On rappelle constit données, idem scirl

-> figure sans classif et zoom (utiliser librairie spy de tikz dès que j'ai le temps)

-> p-valeurs

-> On peut rien dire sur la première valeur, mais ensuite CSI est clairement au dessus

** Conclusion 
   La validation théorique est plus sympa que pour scirl puisqu'on maitrise mieux les termes d'erreur.

   Empiriquement Cascading a des performances similaires à SCIRL, même légèrement supérieures. Cela valide l'heuristique et la logique qui est derrière.

   CSI et est encore plus flexible, puisque des méthodes supervisées non paramétriques (ou à détermination automatique de paramètres) peuvent être employées, ce qui en plus de résoudre les problèmes structurels de PIRL comme le fait déjà SCIRL, résout les problèmes plus fondamentaux de l'approximation linéaire de la fonction de valeur ou du choix des attributs.
