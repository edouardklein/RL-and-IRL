...
# ?cs 3 valueeval 'V^{#1}_{#2}\left({#3}\right)' 'V^{\pi}_R(s)' 'Valeur de l{\apos}état $s$ pour la récompense $R$ lorsqu{\apos}on suit la politique $\pi$'
...
\begin{eqnarray}
\label{eq:Vdefsum}
\valueeval{\policy}{\rewardfunc}{\state_{0}}&=&\sum\limits_{\timeindex = 0}^{\timehorizon} \discount^{\timeindex}\staterewardfunceval{\state_{\timeindex}}\\
...
\begin{equation}
\forall \state, \valueeval{\optimalpolicy{\rewardfunc}}{\rewardfunc}{\state} \geq \valueeval{\policy}{\rewardfunc}{\state}.
\end{equation}
...
\begin{equation}
\valueeval{\policy}{\rewardfunc}{\state} = \staterewardfunceval{\state} + \discount \sum_{\state'\in \statespace}\transprobfunceval{\state}{\policy(\state)}{\state'} \valueeval{\policy}{\rewardfunc}{\state'}.
\end{equation}
...
# Optimisation de Q locale => optimisation de V globale
