   ...
   - Pour le nombre d'itérations prévues :
     ...
     - Effectuer la mise à jour $\theta_{t+1} = \theta_t -\alpha_t {1\over N}\Delta\theta$
       #+begin_src python :tangle LAFEM.py
         ...
         theta = theta - self.alpha( t ) * DeltaTheta / scipy.linalg.norm(DeltaTheta)
         sys.stderr.write("Norme du gradient : "+str(scipy.linalg.norm(DeltaTheta))+"\n")
         if scipy.linalg.norm( DeltaTheta ) < self.Threshold:
             break
       #+end_src
   - retourner $\theta_T$
     #+begin_src python :tangle LAFEM.py
