...
\item $\Delta q$, defined by 
  ...
  \end{itemize}
Readers familiar with the work presented in \cite{klein2012scirl} will see some similarities between the theoretical analyses of SCIRL and CSI. As both algorithms solve the same problem, both theorems naturally bound the same criterion. Another shared feature is the use of the score function $q$ of the classifier as a proxy for the action-value function of the expert $Q^{\pi_E}_{R_E}$. The same structure can therefore be found in both analyses as there is only so many ways to study error propagation in IRL algorithms.\\
