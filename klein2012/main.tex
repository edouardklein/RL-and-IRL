...
\end{equation}
est convergente vers un minimiseur $\theta^*$ de $J_N$, où $\alpha_t$ est un taux d'apprentissage satisfaisant les condition usuelles ($\sum_{t>0} \alpha_t$ diverge et $\sum_{t>0}\alpha_t^2$ converge). Ce minimiseur vérifiera l'équation~\eqref{eq:existence} car $M^*\subset\hat{M}$ et la politique de l'expert sera optimale pour la récompense relative à $\theta^*$ car l'équation~\eqref{eq:existence} implique l'équation d'optimalité de Bellman.
