...
telle que :
\begin{align}
&&\forall \bk\in\mathbb{N}, \alpha_{\bk}>0,
\\
...
\end{equation}
et cela nous assurera que ~$\hat{\theta}_x\in M^*$ d'après la minoration ~$R_N(\theta)\geq {1\over N} \sum_{i=1}^N l(s_i,a_i),~\forall~\theta~\in~\mathbb{R}^p$.
\begin{preuve}
...
\begin{equation}
\forall i\in\{1,\dots,N\}, \forall a \in A \backslash a_i, l(s_i,a)=c_i=\max_{a\in A \backslash a_i}l(s_i,a)>l(s_i,a_i)\geq0, \text{ où $c_i$ est une constante réelle,} 
\end{equation}
...
Minorons les $\beta_i$ :
\begin{equation}
\forall i\in\{1,\dots,N\}, \beta_i=\hat{\theta}_1^T\mu_E(s_i,a_i)-\max_{A \backslash a_i}\hat{\theta}_1^T\mu_E(s_i,a)\geq|\|\mu_E(s_i,a_i)\|_2-\max_{A \backslash a_i}\|\mu_E(s_i,a)\|_2|.
\end{equation}
Donc :
\begin{align}
&R_N(\hat{\theta}_x)={1\over N} \sum_{i=1}^N l(s_i,a_i),
...
\nonumber\Uparrow
\\
&\forall i\in\{1,\dots,N\}, x\geq\frac{\max_{a\in A \backslash a_i}l(s_i,a)-l(s_i,a_i)}{|\|\mu_E(s_i,a_i)\|_2-\max_{A \backslash a_i}\|\mu_E(s_i,a)\|_2|}.
\end{align}
