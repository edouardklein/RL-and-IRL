...
\shorttitle{Classification structurée pour l'IRL}
...
\end{equation}
où $\gamma$ est un facteur d'actualisation positif strictement plus petit que $1$. Il est également possible de définir une fonction voisine de la fonction de valeur, la fonction de qualité :
\begin{equation}
\label{Qdef.eqn}
Q^\pi(s,a) = E\left[\left.\sum_t\gamma^tR(s_t)\right|s_0=s,a_0=a,\pi\right] = R(s) + \gamma E[Q^\pi(s',\pi(s'))|s,a].
\end{equation}
  Une politique optimale $\pi^*$ est définie comme une politique dont la fonction de valeur $V^*$ (appelée fonction de valeur optimale) vérifie :
\begin{equation}
...
%%%%%
\section{Classification structurée pour l'apprentissage par renforcement inverse (SCIRL)}
%%%%%
%%%
\subsection{Principe}
%%%
