...
We define both feature functions : $\phi: S\times A \rightarrow \mathbb{R}^p$ and $\phi: S\rightarrow \mathbb{R}^p$. FIXME: deux fois p, il devrait y avoir une fois p une fois k
#+begin_src c :tangle plot.c :main no
