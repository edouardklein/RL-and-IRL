...
#+BIBLIOGRAPHY: ../Biblio/Biblio plainnat limit:t

#+LATEX_HEADER: \usepackage{amsmath}
#+LATEX_HEADER: \usepackage{amsthm}
#+LaTeX_HEADER: \newtheorem{definition}{Definition}
#+LaTeX_HEADER: \usepackage{natbib}
#+TITLE:Avancement biblio
...
** Learning from advice
   In order to speed up learning, work has been done in order to give more information than just the reward.

   \citet{korupolu2011beyond} present a framework for learning from sporadic instructions from a human.
** Policy search
   Un algo de recherche par MCMC dans l'espace des politiques assez intéressant est décrit dans \citep{wingate2011bayesian}. On  peut introduire de l'information par les priors.
* IRL
** Overview
...
*** MMP
    ...
    Dans la même veine, l'algorithme TBRIL citep{natarajan2011imitation} s'occupe des domaines relationels. Le but annoncé est d'apprendre une politique, mais pas forcément d'améliorer ensuite par rapport à la récompense induite, car c'est trop compliqué (exemple des échecs).
...
*** Maximum Entropy IRL
    ...
    A follow up is \citep{boularias2011relative}, which use the relative entropy between state-action distributions of policies to lead a search without having to solve the MDP problem at each step. Ca reste de l'imitation et non du vrai IRL.
...
** Other approaches
   ...
   Un autre algo Bayesien, G-MH a été publié dans \citep{rothkopf2008modular}. Cette approche généralise le travail de \citet{ramachandran2007bayesian}. Elle est poursuivie dans \citep{dimitrakakis2011bayesian}, où elle est étendue au cas à plusieurs experts (qui font différentes tâches, ou différemment la même tâche).



   Le cadres des MDP à récompense incertaine (IRMDP) ressemble un peu à l'IRL. On cherche la politique otpimal sachant qu'on ne connaît pas vraiment la récompense. Une approche minimax (ils parlent aussi du maximin) robuste (à relier à \citep{syed2008game}) peut être trouvée dans \citep{regan2011robust}. 
...
* Feature Selection
  ...
  La régularisation $l_1$ des paramètres trouvés par LSTD pour l'approximation de la fonction de valeur donne lieu à LARS-TD, présenté en \citep{kolter2009regularization}. Le vecteur de paramètre étant sparse, les paramètres non nuls choisissent les features intéressantes.
* Feature Learning
...
** TL and RL
   ...
   Dans le cas multi agent, l'algorithme BITER \citep{boutsioukis2011transfer} propose une solution basée sur un biais de la valeur initiale de V ou Q dans l'algorithme d'apprentissage par renforcement.



   Ce biais est équivaent à l'utilisation d'une fonction de shaping de la reward. Ce point, ainsi que l'importance de la sélection des features dans la représentation de la fonction de shaping est discuté dans \citep{snel2011multi}, qui ne présente malheureusement pas d'algorithmes utilisables pour résoudre les problèmes soulevés.
* Classification
  Un papier de \citet{weinberger2010convex} présente une amélioration des kNN et des gaussian mixture models basée sur l'apprentissage d'une nouvelle distance. Ca reste du supervisé.
* Theoretical tools
