\documentclass{jfpda2013}

\newcommand{\verbfichex}{jfpda2013.\xspace}
\newcommand{\verbfichclass}{jfpda2013.cls\xspace}
\newcommand{\verbfichbst}{jfpda2013.bst\xspace}
...
\shorttitle{Approche cascadée de l'ARI}
...
\shortouvrage{JFPDA 2013}

% Titre, auteur, pas de date

\title{Apprentissage par renforcement inverse en cascadant classification et régression
}
...
\begin{abstract}
  Cette contribution traite du problème de l'apprentissage par renforcement inverse, c'est à dire la découverte d'une fonction de récompense pour laquelle la politique de l'expert que l'on observe est optimale.
Nous poposons de diviser le problème en deux étapes d'apprentissage supervisé : c'est l'approche en cascade (CSI pour {\it Cascaded Supervised Inverse reinforcement learning}). Une étape de classification qui définit une fonction de score est suivie par une étape de régression qui donne une fonction de récompense.
Une analyse théorique démondre que la politique de l'expert observée est quasi-optimale pour la fonction de récompense trouvée.
Deux avantages de cette approche sont de ne pas avoir recours à la résolution répétée du Processus Decisionnel de Markov (MDP) et la possibilité de profiter des techniques existantes en classification et régression. Nous montrons de plus empiriquement que CSI peut fonctionner avec pour seule information (à une heuristique près) des échantillons recueillis par la politique de l'expert. Nous testons cela sur deux banc d'essais classiques, le {\it mountain car} et un simulateur de conduite sur autoroute.
  \end{abstract}
...
\begin{table}
	\begin{center}
		...
		\end{tabular}
	\end{center}
        \caption{Student or Welch test of mean equality (depending on wether a Bartlett test of variance equality succeeds) $p$-values for CSI and SCIRL on the mountain car. High values ($>1.0\e{-02}$) means that the hypothesis that the means are equal cannot be rejected. As highlighted here, the means are different when the algorithms are fed only $10$ expert samples in which case CSI outperforms SCIRL.}
        \label{tab:mc}
\end{table}
\begin{figure}
...
\begin{table}[ht]
	\begin{center}
		...
		\end{tabular}
	\end{center}
        \caption{Student or Welch test of mean equality (depending on wether a Bartlett test of variance equality succeeds) $p$-values for CSI and SCIRL on the driving simulator. High values ($>1.0\e{-02}$) means that the hypothesis that the means are equal cannot be rejected. As highlighted here, the means are different from 49 samples on : CSI slightly but definitively outperforms SCIRL.}
        \label{tab:highway}
\end{table}
  \begin{figure}
